...
rfDetector = RetinaFace(os.path.join(os.path.dirname(__file__),'../RetinaFace/model/R50'), 0, 0, 'net3')
...
def align_pic_new(image, threshold, model = 'retina'):
    ...
    if model == 'dlib':
        ...
        rects, landmarks = rfDetector.detect(image, 0.8, scales=[1.0], do_flip=False)
        if len(rects) == 0:
          print('no face')
          return None
        rf_flag = True
        l = landmarks.reshape(landmarks.shape[0], 2, 5)
        l_x, l_y = [], []
        ...
        l_x = np.array(l_x)
        l_y = np.array(l_y)
        l_x = np.logical_and(l_x > 0, l_x < shape[0]).all(axis=1)
        l_y = np.logical_and(l_y > 0, l_y < shape[1]).all(axis=1)
        mask = np.logical_and(l_x, l_y)
        rects = rects[mask] 
        #print('rect len:', len(rects))
    else:
        rects = detector(image, 2)
        if len(rects) == 0:
            rects, _ = rfDetector.detect(image, 0.8, scales=[1.0], do_flip=False)
...
def align_pics(imgPath, rootDir = './output/', output = True, model = 'retina'):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--root", default="./output/", help="root directory for storing aligned images")
